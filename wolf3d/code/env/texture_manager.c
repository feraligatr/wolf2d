...
#ifdef IPHONE
		pfglGenerateMipmapOES( GL_TEXTURE_2D );
#else		
		if( tex->MipMap )
		{
			int miplevel = 0;
...
// the wall exporter always saved as 5551 even when there was no alpha
		...
		while( 1 ) {
			...
			if ( fi->bpp < 16 ) {
				...
				pfglCompressedTexImage2D( GL_TEXTURE_2D, l, fi->internalFormat, w, h, 0, 
							   size, s );
			} else {
				pfglTexImage2D( GL_TEXTURE_2D, l, fi->internalFormat, w, h, 0, 
